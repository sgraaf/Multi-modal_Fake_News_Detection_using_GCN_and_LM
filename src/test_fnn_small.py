...
def test():
    ...
    FNN_small_test = FNNDataset(data_dir / ('FNN_small_test.pkl'), GloVe_vectors, ELMo)
    FNN_DL_small_test = data.DataLoader(
                dataset=FNN_small_test,
    ...
    for step, batch in enumerate(FNN_DL_small_test):       
        ...
        if step % 100 == 0 and step != 0:
            print(sum(1 for x,y in zip(y_pred,y_true) if x == y) / len(y_pred))
    ...
    print(sklearn.metrics.precision_recall_fscore_support(y_true, y_pred, average=None))
    #pkl.dump(embeds, open(data_dir / 'FNN_small_embeds.pkl', 'wb'))
