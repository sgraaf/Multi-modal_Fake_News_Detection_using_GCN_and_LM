...
for i in range(len(user_art_buz)):
    adj_art_user[art_dict_buz[user_art_buz[i,0]], user_art_buz[i,1]] = 1
for i in range(len(user_art_pol)):
    adj_art_user[art_dict_pol[user_art_pol[i,0]], user_art_pol[i,1]] = 1
    
adj_art_user2 = np.zeros((35321, 422))
for i in range(len(user_art_buz)):
    adj_art_user2[user_art_buz[i,1], art_dict_buz[user_art_buz[i,0]]] = 1    
for i in range(len(user_art_pol)):
    adj_art_user2[user_art_pol[i,1], art_dict_pol[user_art_pol[i,0]]] = 1
...
np.savetxt(path_output, adj_matrix, delimiter=",")
